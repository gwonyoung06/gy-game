...
- [x] 스킬 포획 콤보 버그 수정
- [x] index.html 누락 UI 요소 전부 추가 (Batch 13) — 스트릭·업적·이름표·피드·터치컨트롤·페이드오버레이
- [x] HUD.js dead code 제거 + null guard 강화
- [x] _cleanup() combo-glow·timer-urgency 리셋
...
- **vercel.json에 `"framework"` 필드 절대 추가 금지** → "Invalid vercel.json" 빌드 실패 발생
- **vercel.json 허용 필드**: `buildCommand`, `outputDirectory`, `installCommand` 3개만

### 올바른 vercel.json (이 형태 고정, 절대 변경 금지)
\`\`\`json
{
  "buildCommand": "cd game && npm run build",
  "outputDirectory": "game/dist",
